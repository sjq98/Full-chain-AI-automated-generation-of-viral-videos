...
import { BROWSER_ARGS, PLAYWRIGHT_DEFAULT_ARGS_TO_IGNORE, TIMEOUTS, PUBLISH_SELECTORS, URLS } from '../constants.js';
import { keepOnlyPage, prepareSingleVisiblePage, restoreVisibleWindow } from '../browser-window.js';
...
export class PublishService {
  ...
  async publishContent(params: PublishContentParams): Promise<PublishResult> {
    ...
    if (!chromeExecutable) {
      throw new Error('Google Chrome is required for Xiaohongshu publishing.');
    }
    ...
    const page = await prepareSingleVisiblePage(this.ctx.context, URLS.PUBLISH);
    ...
    try {
      ...
      await keepOnlyPage(this.ctx.context, page);
  ...
  async publishVideo(params: PublishVideoParams): Promise<PublishResult> {
    ...
    const page = await prepareSingleVisiblePage(this.ctx.context, publishUrl);
