  ...
  switch (session.status) {
    case 'waiting_scan':
      return session.qrCodeUrl
        ? 'Show QR code URL to user. After scanning, call xhs_check_login_session with this sessionId.'
        : 'Ask the user to scan the QR code in the visible browser window, then call xhs_check_login_session with this sessionId.';
    case 'scanned':
