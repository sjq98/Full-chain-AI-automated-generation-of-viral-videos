...
import { BROWSER_ARGS, PLAYWRIGHT_DEFAULT_ARGS_TO_IGNORE, QR_CODE_SELECTOR, LOGIN_STATUS_SELECTOR, URLS } from '../xhs/clients/constants.js';
import { keepOnlyPage, prepareSingleVisiblePage, restoreVisibleWindow } from '../xhs/clients/browser-window.js';
...
export class LoginSessionManager {
  ...
  async createSession(accountName?: string, proxy?: string): Promise<LoginSession> {
    ...
    // Launch browser
    const chromeExecutable = process.env.XHS_MCP_CHROME_EXECUTABLE;
    if (!chromeExecutable) {
      throw new Error('Google Chrome is required for Xiaohongshu login.');
    }
    const launchOptions: any = {
      headless: config.browser.headless, // 可通过 XHS_MCP_HEADLESS 控制
      executablePath: chromeExecutable,
      ignoreDefaultArgs: PLAYWRIGHT_DEFAULT_ARGS_TO_IGNORE,
      args: BROWSER_ARGS,
    };
    ...
    const page = await prepareSingleVisiblePage(context, URLS.EXPLORE);
    ...
    // 尝试从页面状态提取 QR 码数据。小红书会不定期调整前端状态结构，
    // 但可见浏览器里的二维码本身仍可供用户扫码，因此不能因链接提取失败
    // 就关闭浏览器并把登录准备判为失败。
    ...
    if (loginData?.qrcode) {
      ...
      log.warn('QR code state was unavailable; keep the visible login page open for direct scanning');
    }
